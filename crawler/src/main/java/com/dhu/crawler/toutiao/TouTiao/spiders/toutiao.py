...
def get_page_index(start_url, offset, keyword):# 根据参数生成初始索引页URL的正确格式
    data = {
        'aid': '24',
        'app_name': 'web_search',
        'offset': offset,
...
class ToutiaoSpider(scrapy.Spider):# 头条spider类，进行数据采集工作
    name = 'toutiao'
        ...
        if dic and 'data' in dic.keys():  # dic数据中有data数据段
            ...
            for node in dic.get('data'):  # data中的每个结点object
                ...
                try:#将结点中的数据值付给对应的item
                    ...
                    request = urllib.request.Request(url, headers=headers)# 破解headers反爬
                    response = urllib.request.urlopen(request)
                    html = str(response.read(), 'utf-8')
                    content = re.findall(r"content: '(.+);',", str(html))
                    # 将发布方以及评论数反序列化json字符串，存入数据库information字段
